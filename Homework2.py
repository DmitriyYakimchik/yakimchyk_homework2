...
# тут и далее получил у себя ошибку TypeError: unsupported operand type(s) for |: 'type' and 'type'
# Описание ошибки:
# Эта ошибка возникает из-за того, что оператор | используется для объединения типов в аннотации возвращаемого значения функции. 
# Однако этот оператор доступен только в Python 3.10 и выше. 
# В версиях Python до 3.10 вы можете использовать Union из модуля typing для объединения типов в аннотации возвращаемого значения функции.
# У меня Python 3.10, но оператор | работать отказался
# Это не твоя ошибка, просто знай, что на системах с более старыми версиями питона код не запустится
...
def common_price(m: int, n: int, s: int, l: int) -> bool | str:
    ...
    if type(m) != int or type(n) != int or type(s) != int or type(l) != int:
        ...
        # Круто, что отпечатывается в консоль пояснение ошибок
        print('Zero goods')
...
# Хорошее рабочее решение, для альтернативного, рекомендую посмотреть ответы
